...
export default function Workspace() {
  ...
  async function loadWarmupQuestions() {
    if (!id) return;
    try {
      // Workspace-type still comes from the agent — pull it in
      // parallel with the merged warm-ups list, which is now the
      // union of agent.suggested_questions (manually-typed in the
      // agent-settings modal) and SourceWarmup rows for the active
      // sources (generated by the source-onboarding flow).
      const [merged, agent] = await Promise.all([
        dataClient.listAgentWarmupQuestions(id),
        dataClient.getAgent(id),
      ]);
      setWarmupQuestions(merged?.questions || []);
