...
async def generate_table_summary_csv(
    ...
    system_report = (
        ...
        "what this CSV/spreadsheet contains, based ONLY on: the column list, the sample profile (types, nulls, "
        "numeric stats, top values), total row count, and a tiny sample of up to 5 rows. "
        "Do not assume you have the full dataset. Keep it concise (1-2 pages when rendered). "
        "Include: what the data represents, key columns, data quality notes, and notable patterns from the profile. "
        "Use clear headings and bullet points. Write in the same language as the data (e.g. Portuguese if sample is in Portuguese)."
    )
    user_report = (
